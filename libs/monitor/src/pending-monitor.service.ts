...
export class PendingMonitor {
  ...
  async poll(): Promise<void> {
    ...
    this.logger.log(
      `PendingMonitor: processing ${rows.length} broadcasted signing_request(s)`,
    );
  ...
  private async processRow(
    row: Awaited<
      ReturnType<SigningRequestRepository['findBroadcasted']>
    >[number],
  ): Promise<void> {
    ...
    try {
      receipt = await this.gasService.getTransactionReceipt(
        txHash,
        row.chain_id,
      );
    } catch (err) {
