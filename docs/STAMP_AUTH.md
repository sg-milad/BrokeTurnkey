...
- **Public key** — sent to WalletMVP once during key registration
  (`POST /api-keys`) in PEM format. Stored in `api_keys.public_key`.
...
sequenceDiagram
    ...
    Client->>API: POST /api-keys (X-Stamp with existing key, or root token for first key)
...
Resolution: `POST /api-keys` accepts a **one-time
...
2. Register the public key via `POST /api-keys` using the
